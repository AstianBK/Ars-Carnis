...
import com.TBK.ars_carnis.server.network.messager.PacketSyncCooldown;
...
public class PlayerCooldowns {
    ...
    public void addCooldown(PartAbstract power, int durationTicks) {
        powerCooldowns.put(power.name, new CooldownInstance(durationTicks));
    }

    public void addCooldown(PartAbstract power, int durationTicks, int remaining) {
    ...
    public boolean isOnCooldown(PartAbstract power) {
        return powerCooldowns.containsKey(power.name);
    }

    public float getCooldownPercent(PartAbstract power) {
